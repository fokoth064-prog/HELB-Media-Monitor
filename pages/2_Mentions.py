...
def load_data(csv_url):
    ...
    if "published" in df.columns:
        ...
        try:
            ...
            # If tz_convert fails (naive datetimes), ignore
            pass
...
# Utility: build HTML table with conditional row colors
def make_html_table(df, cols, max_rows=200):
    ...
    - Neutral  -> light grey
    - Negative -> light red
    Links are rendered as clickable anchors (open in new tab).
    """
    # Color mapping
    COLORS = {
...
# Filters: small sidebar
st.sidebar.header("Filters / Keyword Trends")
search = st.sidebar.text_input("Search (TITLE / SUMMARY)")
min_date = df["DATE"].min() if df["DATE"].dtype == object else ""
max_date = df["DATE"].max() if df["DATE"].dtype == object else ""
# Note: keep the existing date range widget optional; we focus on keyword filters here
